...
def read_wildguard_for_bt(input_path, data_type='train'):
    ...
    label = [1 if label == 'harmful' else 0 for label in df.prompt_harm_label.tolist()]
    print(f'WildGuard {data_type} has {len(ori_sen)} samples.')
    return ori_sen, label


def preprocess(dataset='aclImdb',dst_path='../data/aclImdb/', src_path='aclImdb/'):
    s_t = time.time()
    if os.path.exists(dst_path):
        shutil.rmtree(dst_path)
    os.mkdir(dst_path)
    s1,l1 = read_aclImdb(src_path, data_type='train')
    s1,l1,s1d,l1d = select_data(dataset,s1,l1,data_type = 'train')
    s2,l2 = read_aclImdb(src_path, data_type='test')
...
def preprocess_wildguard(dataset='wildguardmix_HT',dst_path='./data/wildguardmix_HT/', src_path='/data/wildguardmix_HT_init/'):
    s_t = time.time()
    # if os.path.exists(dst_path):
    #     shutil.rmtree(dst_path)
    # os.mkdir(dst_path)
    ...
    s1,l1 = read_wildguard_for_bt(src_path, 'train')
    s1d,l1d = read_wildguard_for_bt(src_path, 'dev')
    s2,l2 = read_wildguard_for_bt(src_path, 'test')

    print(f'Starting test after {(time.time()-s_t)/3600} hours!')
    make_json_file(s2,l2,dst_path,data_type='test')
    print(f'Starting dev after {(time.time()-s_t)/3600} hours!')
    make_json_file(s1d,l1d,dst_path,data_type='dev')
    print(f'Starting train after {(time.time()-s_t)/3600} hours!')
    make_json_file(s1,l1,dst_path,data_type='train')
    e_t = time.time()
    print(dataset,' costs ',(e_t-s_t)/3600,' hours!')
...
start_time = time.time()
os.makedirs('./data/wildguardmix_HT_init/', exist_ok=True)
preprocess_wildguard()
end_time = time.time()
print('In total, it costs ',(end_time-start_time)/3600,' hours!')
