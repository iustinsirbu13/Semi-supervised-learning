...
# from semilearn.algorithms.disaster.multihead_apm.apm_hook_vwithoutapm import APMHookWithoutAPM
...
from semilearn.algorithms.disaster.jointmatch.utils import JointMatchThresholdingHook

import jsonlines
import os

@ALGORITHMS.register('jointmatch')
class JointMatch(AlgorithmBase):
    def __init__(self, args, net_builder, tb_log=None, logger=None):
        self.use_debug = args.use_debug
        
        # multihead specific arguments
        self.num_heads = 2 # num_nets
    ...
    # @overrides
    def set_hooks(self):
        # if self.args.multihead_apm_variant == 'v-apm':
        #     # remove the APM selection strategy; only for the ablation study
        #     self.register_hook(APMHookWithoutAPM(self.args, APMLogHook()), "APMHook")
        # else:
        ...
        for i in range(self.num_heads):
            if self.threshold_algo == 'flexmatch':
                ...
                self.register_hook(FlexMatchLogHook(masking_hook_name=f"MaskingHook{i}"), f"FlexMatchLogHook{i}")
                ...
                self.register_hook(FreeMatchLogHook(masking_hook_name=f"MaskingHook{i}"), f'FreeMatchLogHook{i}')
            elif self.threshold_algo == 'jointmatch':
                self.register_hook(JointMatchThresholdingHook(num_classes=self.num_classes, momentum=self.args.ema_p, fixed_threshold=self.args.p_cutoff), f"MaskingHook{i}")
                self.register_hook(FreeMatchLogHook(masking_hook_name=f"MaskingHook{i}"), f'FreeMatchLogHook{i}')
    ...
    def _get_auxiliary_mask(self, logits_x_ulb_w, idx_ulb, head_id):
        ...
        if self.threshold_algo == 'freematch':
            ...
            mask = torch.ones(idx_ulb.shape[0], dtype=torch.int64).to(self.args.device)
        else:
            raise NotImplementedError()
        return mask
    
    def _log_stats_dict(self, head_id, samples_weights, confidence_mask, pseudo_labels, y_ulb):
        final_weights = samples_weights * confidence_mask
        dw = self.args.disagreement_weight
        my_stats_dict = {
            'head_id': head_id,
            'mask_rate': (final_weights == 0).float().mean().item(),
            'disagreement_rate': (final_weights == dw).float().mean().item(),
            'agreement_rate': (final_weights == 1-dw).float().mean().item(),
            'impurity': (pseudo_labels[head_id][final_weights != 0] == y_ulb[final_weights != 0]).float().mean().item(),
            'disagreement_impurity': (pseudo_labels[head_id][final_weights == dw] == y_ulb[final_weights == dw]).float().mean().item(),
            'agreement_impurity': (pseudo_labels[head_id][final_weights == 1-dw] == y_ulb[final_weights == 1-dw]).float().mean().item(),
        }
    ...
    def get_unsupervised_loss(self, ulb_weak_logits, ulb_strong_logits, pseudo_labels, idx_ulb, y_ulb):
        head_id, other_head_id = 0, 1
        # other_head_id = 1 - head_id
        # num_ulb = idx_ulb.shape[0]

        confidence_mask_mask = self._get_auxiliary_mask(ulb_weak_logits[head_id], idx_ulb, head_id)
        other_confidence_mask = self._get_auxiliary_mask(ulb_weak_logits[other_head_id], idx_ulb, other_head_id)

        disagree_mask = (pseudo_labels[head_id] != pseudo_labels[other_head_id])
        agree_mask = (pseudo_labels[head_id] == pseudo_labels[other_head_id])
        samples_weights = disagree_mask * self.args.disagreement_weight + agree_mask * (1 - self.args.disagreement_weight)

        if self.use_debug:
            self._log_stats_dict(head_id, samples_weights, confidence_mask_mask, pseudo_labels, y_ulb)
            self._log_stats_dict(other_head_id, samples_weights, other_confidence_mask, pseudo_labels, y_ulb)

        loss1 = (F.cross_entropy(ulb_strong_logits[head_id], pseudo_labels[other_head_id], reduction='none') * samples_weights * other_confidence_mask).mean()
        loss2 = (F.cross_entropy(ulb_strong_logits[other_head_id], pseudo_labels[head_id], reduction='none') * samples_weights * confidence_mask_mask).mean()
        head_losses = [loss1, loss2]
        if self.args.average_losses:
            return sum(head_losses) / len(head_losses)
        return sum(head_losses)
    ...
    #     if self.args.aux_mask_operation == 'OR':
    #         auxiliary_mask = torch.maximum(auxiliary_mask1, auxiliary_mask2)
    #     elif self.args.aux_mask_operation == 'AND':
    #         auxiliary_mask = torch.minimum(auxiliary_mask1, auxiliary_mask2)
    #     elif self.args.aux_mask_operation == 'SUM':
    #         auxiliary_mask = auxiliary_mask1 + auxiliary_mask2
    #     else:
    #         raise NotImplementedError()
    ...
    #     if self.args.multihead_apm_variant in ['v4', 'v5', 'v6', 'v8', 'v9', 'v-apm', 'v4-cutoff']:
    ...
    #         return (F.cross_entropy(ulb_strong_logits[head_id], multihead_labels, reduction='none') * samples_weights * auxiliary_mask).mean()

    #     if self.args.apm_disagreement_weight == -1:
    #         # legacy code
    #         mask = multihead_labels != -1
    #         mask = mask * auxiliary_mask
    #         if 1 not in mask:
    ...
    #             _msk = (agreement_types_mask == 0) * auxiliary_mask
    #             ce_disagreement = F.cross_entropy(ulb_strong_logits[head_id][_msk], multihead_labels[_msk]) 
    #         else:
    #             ce_disagreement = torch.tensor(0).to(self.args.device)
    #         if 1 in agreement_types_mask:
    #             _msk = (agreement_types_mask == 1) * auxiliary_mask
    #             ce_agreement = F.cross_entropy(ulb_strong_logits[head_id][_msk], multihead_labels[_msk]) 
    #         else:
    ...
    def load_model(self, load_path):
        ...
        for i in range(self.num_heads):
            if self.threshold_algo == 'freematch':
                self.hooks_dict[f'MaskingHook{i}'].p_model = checkpoint[f'p_model{i}'].cuda(self.args.gpu)
                self.hooks_dict[f'MaskingHook{i}'].time_p = checkpoint[f'time_p{i}'].cuda(self.args.gpu)
            elif self.threshold_algo == 'flexmatch':
    ...
    def get_argument():
        return [
            SSL_Argument('--use_debug', str2bool, True),
            SSL_Argument('--disagreement_weight', float, 1), # in [0, 1] if set
            SSL_Argument('--average_losses', str2bool, False),
            SSL_Argument('--threshold_algo', str, 'freematch'),
            SSL_Argument('--ema_p', float, 0.999), # freematch momentum --> 0.9 in jointmatch
            SSL_Argument('--p_cutoff', float, 0.95), # fixed threshold, 0.98 in jointmatch
            ...
            # SSL_Argument('--apm_update_method', str, "original"),
            # SSL_Argument('--aux_mask_operation', str, 'OR'),
