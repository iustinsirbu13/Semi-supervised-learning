...
class ClassificationNeobert(nn.Module):
    def __init__(self, name, num_classes=15):
        super(ClassificationNeobert, self).__init__()
        # Load pre-trained bert model
        self.bert = AutoModel.from_pretrained(name, trust_remote_code=True)
...
def NeoBERT(pretrained=True, pretrained_path=None, **kwargs):
    print("GOT NEOBERT MODEL\n", )
    model = ClassificationNeobert(name='chandar-lab/NeoBERT', **kwargs)
    return model