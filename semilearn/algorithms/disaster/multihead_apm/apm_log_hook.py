...
class APMLogHook(Hook):
    ...
    def after_train_epoch(self, algorithm):
        ...
        for head_id, head_id1, head_id2 in [(0, 1, 2), (1, 0, 2), (2, 0, 1)]:
            ...
            algorithm.print_fn(f"HEAD[{head_id}]: Agreement percent=f{agreement_percent}")
            ...
            algorithm.print_fn(f"HEAD[{head_id}]: APM included samples per class=f{torch.bincount(hook.apm_labels[head_id][apm_mask])}")

            # Total unlabeled percent used
            algorithm.print_fn(f"HEAD[{head_id}]: Total unlabeled percent=f{agreement_percent + apm_percent}")

            # APM class thresholds for current head
            algorithm.print_fn(f"HEAD[{head_id}]: APM class thresholds=f{hook.apm_cutoff[head_id]}")
