...
def process_split(ds, response_column, cat_column):
    print(ds.shape)
    ds = ds[
            (ds[cat_column] != 'Needs Caution') &
            (ds[cat_column] != 'Other') &
            (ds[response_column].notna())]
    print(ds.shape)
    print(ds[cat_column].value_counts())
    return ds

def format_as_json():
    dst_path = './data/aegis2.0_RH'
    ...
    ds_train = process_split(ds_train, response_column, cat_column)
    print(ds_train.shape)

    ds_valid = process_split(ds_valid, response_column, cat_column)
    print(ds_valid.shape)

    ds_test = process_split(ds_test, response_column, cat_column)
    print(ds_test.shape)
