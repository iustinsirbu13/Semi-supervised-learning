...
def process_split(ds, label_column, cat_column):
    print(ds.shape)
    ds[cat_column] = np.where((ds[label_column].eq('safe')) & (~ds[cat_column].eq('Needs Caution')), 
                                               'Safe',
                                               ds[cat_column])

    ds.loc[:, cat_column] = ds[cat_column].str.split(',').str[0]
    ds = ds[(ds[cat_column].notnull()) & 
            (ds[cat_column] != 'Needs Caution') &
            (ds[cat_column].str.len() > 0) &
            (ds[cat_column] != 'Other')]
    print(ds.shape)
...
def format_as_json():
    dst_path = './data/aegis2.0_PH_multiclass'
    text_column = 'prompt'
    ...
    labels = {
        ...
        "Unauthorized Advice": 9,
        "Controlled/Regulated Substances": 10,
        "Guns and Illegal Weapons": 11,
        "Political/Misinformation/Conspiracy": 12,
        "Fraud/Deception": 13,
        "Sexual (minor)": 14,
        "Illegal Activity": 15,
        "Malware": 16,
        "Immoral/Unethical": 17,
        "Threat": 18,
        "Copyright/Trademark/Plagiarism": 19,
        "High Risk Gov Decision Making": 20,
        "Manipulation": 21
    }
    ...
    for split_name, split_ds in datasets.items():
        ...
        with open(os.path.join(dst_path, f'{split_name}.json'), 'w') as outfile:
            ...
            for idx, (index, elem) in enumerate(tqdm(filtered_ds.iterrows(), total=len(filtered_ds), desc=f'Processing {split_name}')):
                ...
                try:
                    data[str(idx)]['label'] = str(labels[elem[cat_column]])
