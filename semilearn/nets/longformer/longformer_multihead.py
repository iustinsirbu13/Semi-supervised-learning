...
class ClassificationLongformerMultihead(nn.Module):
    def __init__(self, name, num_classes=2, num_heads=3, adjust_clf_size=False):
        super(ClassificationLongformerMultihead, self).__init__()
    ...
    def forward(self, x, only_fc=False, only_feat=False, return_embed=False, **kwargs):
        ...
        out_dict = self.longformer(**x, output_hidden_states=True, return_dict=True)
    ...
    def group_matcher(self, coarse=False, prefix=''):
        matcher = dict(stem=r'^{}longformer.embeddings'.format(prefix), blocks=r'^{}longformer.encoder.layer.(\d+)'.format(prefix))
        return matcher
...
def longformer_base_multihead(args, **kwargs):
    model = ClassificationLongformerMultihead('allenai/longformer-base-4096', args.num_classes, args.num_heads, args.adjust_clf_size, **kwargs)
    return model


def longformer_large_multihead(args, **kwargs):
    model = ClassificationLongformerMultihead('allenai/longformer-large-4096', args.num_classes, args.num_heads, args.adjust_clf_size, **kwargs)
    return model
