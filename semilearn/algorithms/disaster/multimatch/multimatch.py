...
from semilearn.algorithms.disaster.multimatch.multimatch_hook import MultiMatchHook
from semilearn.algorithms.disaster.multimatch.multimatch_hook_withoutAPM import MultiMatchHookWithoutAPM
from semilearn.algorithms.disaster.multimatch.apm_log_hook import APMLogHook
from semilearn.algorithms.disaster.multimatch.debug_hook import DebugHook
...
class MultiMatch(AlgorithmBase):
    ...
    def set_hooks(self):
        if self.args.multihead_apm_variant == 'v-apm':
            ...
            self.register_hook(MultiMatchHookWithoutAPM(self.args, APMLogHook()), "APMHook")
        else:
            self.register_hook(MultiMatchHook(self.args, APMLogHook()), "APMHook")
        if self.use_debug:
    ...
    def get_head_unsupervised_loss_v2(self, ulb_weak_logits, ulb_strong_logits, pseudo_labels, idx_ulb, y_ulb, head_id):
        '''
        ...
        if self.args.multihead_apm_variant in ['v4', 'v5', 'v6', 'v8', 'v9', 'v-apm', 'v4-cutoff']:
            ...
            wd = self.args.apm_disagreement_weight
            samples_weights = (agreement_types_mask == 0) * wd + (agreement_types_mask == 1) * 1
            ...
            if self.args.save_pseudolabels_stats:
                my_stats_dict = {
                    ...
                    'easy_rate': (final_weights == 1).float().mean().item(),
                    'hard_rate': (final_weights == wd).float().mean().item(),
                    'aux_mask_rate': (auxiliary_mask == 0).float().mean().item(),
                    'apm_mask_rate': (agreement_types_mask == -1).float().mean().item(),
                    'apm_easy_rate': (agreement_types_mask == 1).float().mean().item(),
                    'apm_hard_rate': (agreement_types_mask == 0).float().mean().item(),
                    'impurity': (multihead_labels[final_weights != 0] == y_ulb[final_weights != 0]).float().mean().item(),
                    'easy_impurity': (multihead_labels[final_weights == 1] == y_ulb[final_weights == 1]).float().mean().item(),
                    'hard_impurity': (multihead_labels[final_weights == wd] == y_ulb[final_weights == wd]).float().mean().item(),
                }
                # if self.epoch == 1 and my_stats_dict['impurity'] < 1:
                #     print('final_weights', final_weights)
                #     print('agreement_types_mask', agreement_types_mask)
                #     print('multihead_labels', multihead_labels)
                #     print('y_ulb', y_ulb)
                #     print('my_stats_dict', my_stats_dict)
                #     assert False
    ...
    def get_unsupervised_loss(self, ulb_weak_logits, ulb_strong_logits, pseudo_labels, idx_ulb, y_ulb):
        ...
        head_losses = [self.get_head_unsupervised_loss_v2(ulb_weak_logits, ulb_strong_logits, pseudo_labels, idx_ulb, y_ulb, head_id) for head_id in range(self.num_heads)]
