...
def augment(data, batch_texts, batch_keys):
    try:
        # Create a list of formatted prompt strings for each example in the batch
        batch_prompts = []
        ...
        # Start the timer for monitoring performance
        start_time = time.time()
        
        # Tokenize the batch using plain text prompts, ensuring attention_mask is returned
        encodings = tokenizer(
            batch_prompts,
            return_tensors="pt",
            padding=True,
        ...
        # Move all input tensors to the designated device (GPU if available)
        model_inputs = {key: tensor.to(device) for key, tensor in encodings.items()}
        model.to(device)
        
        # Generate outputs with controlled randomness
        generated_ids = model.generate(
            input_ids=model_inputs['input_ids'],
            attention_mask=model_inputs['attention_mask'],
            max_new_tokens=1000,
            do_sample=True,
            temperature=0.5,  # Adjust this lower for more deterministic output
            top_p=0.9
        )
        ...
        # Post-process outputs to extract only the text after the "FINAL_OUTPUT:" delimiter
        final_outputs = []
        ...
        # Map the generated output back to each key in the data dictionary
